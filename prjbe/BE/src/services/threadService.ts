...
export default new (class ThreadService {
  ...
  async findById(req: Request, res: Response): Promise<Response> {
    try {
      ...
      if (threadDetails) {
        return res.status(200).json({ message: "Success", data: threadDetails })
      } else {
        return res.status(404).json({ message: "Thread not found" })
      }
    } catch (error) {
      console.error("Error during thread retrieval:", error)
      return res.status(500).json({ message: "Error" })
    }
  }
  async findThreadsByUserId(req: Request, res: Response): Promise<Response> {
    try {
